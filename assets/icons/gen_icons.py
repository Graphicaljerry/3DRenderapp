#!/usr/bin/env python3
"""Generate the minimalist icon set (SVGs + preview grid + Figma data) for the app.
...
Outputs:
  - <name>.svg         one file per icon (currentColor, drop-in for code)
  - preview.html       responsive wrap grid (light + dark), mirrors a Figma auto-layout grid
  - _icons.js          `const ICONS = [...]` literal (inner paths) for the Figma importer
"""
import json
import os
...
ICONS = [
    ...
    ("Brand marks", "logo-extrude", "Words to object",
    ...
    ("Create & AI", "wand", "Refine",
     '<path d="M5 19 14 10"/><path d="M15 4l1 2.2 2.2 1-2.2 1-1 2.2-1-2.2-2.2-1 2.2-1z"/><path d="M19.6 9.4l.6 1.4 1.4.6-1.4.6-.6 1.4-.6-1.4-1.4-.6 1.4-.6z"/>'),
    ("Create & AI", "image-to-3d", "Image to 3D",
     '<rect x="4" y="4" width="13" height="13" rx="1.5"/><circle cx="8" cy="8" r="1.4"/><path d="M4 14l4-3 3 2.5"/><path d="M14 20 18 22 22 20 22 15 18 13 14 15Z"/><path d="M14 15 18 17 22 15"/><path d="M18 17V22"/>'),
    ("Create & AI", "variations", "Variations",
     '<rect x="3.5" y="3.5" width="7" height="7" rx="1"/><rect x="13.5" y="3.5" width="7" height="7" rx="1"/><rect x="3.5" y="13.5" width="7" height="7" rx="1"/><path d="M14 17h6"/><path d="M17 14v6"/>'),
    ("Create & AI", "enhance", "Enhance prompt",
     '<path d="M4 6h10"/><path d="M4 11h7"/><path d="M4 16h9"/><path d="M18 12.5l.9 2.1 2.1.9-2.1.9-.9 2.1-.9-2.1-2.1-.9 2.1-.9z"/>'),
    ...
    # ---- Camera & view ----
    ("Camera & view", "hand", "Pan (grab)",
     '<path d="M8 11V6.5a1.5 1.5 0 0 1 3 0V10"/><path d="M11 10V5.5a1.5 1.5 0 0 1 3 0V10"/><path d="M14 10.5V7a1.5 1.5 0 0 1 3 0v5"/><path d="M17 9.5a1.5 1.5 0 0 1 3 0V15a6 6 0 0 1-6 6h-2a6 6 0 0 1-4.2-1.8L5 16a1.5 1.5 0 0 1 2.1-2.1L9 16"/>'),
    ("Camera & view", "reset-view", "Reset view",
     '<circle cx="12" cy="12" r="7"/><path d="M12 2v3"/><path d="M12 19v3"/><path d="M2 12h3"/><path d="M19 12h3"/><circle cx="12" cy="12" r="1.5"/>'),
    ("Camera & view", "grid-floor", "Floor grid",
     '<path d="M2 20h20"/><path d="M5 20 9 8"/><path d="M19 20 15 8"/><path d="M9 8h6"/><path d="M4 16h16"/><path d="M6.5 12h11"/>'),
    ("Camera & view", "axis", "Axes (XYZ)",
     '<path d="M6 18h13"/><path d="M17 16l2 2-2 2"/><path d="M6 18V5"/><path d="M4 7l2-2 2 2"/><path d="M6 18l7 3.5"/>'),
    ("Camera & view", "measure", "Measure",
     '<path d="M3 7v10"/><path d="M21 7v10"/><path d="M3 12h18"/><path d="M6 9l-3 3 3 3"/><path d="M18 9l3 3-3 3"/>'),
    ...
    ("Edit", "rotate", "Rotate",
     '<path d="M12 4a8 8 0 1 1-7.6 5.5"/><path d="M4 4v5h5"/>'),
    ("Edit", "mirror", "Mirror",
     '<path d="M12 3v18"/><path d="M8 6 4 12 8 18Z"/><path d="M16 6 20 12 16 18Z"/>'),
    ("Edit", "extrude", "Extrude",
     '<path d="M4 11h8v8H4z"/><path d="M4 11 8 7h8v8l-4 4"/><path d="M12 11 16 7"/>'),
    ("Edit", "bevel", "Bevel / chamfer",
     '<path d="M9 4h11v11"/><path d="M4 9 9 4"/><path d="M20 15 15 20"/><path d="M4 9v11h11"/>'),
    ("Edit", "hollow", "Hollow / shell",
     '<rect x="4" y="4" width="16" height="16" rx="1"/><rect x="8.5" y="8.5" width="7" height="7" rx="0.5"/>'),
    ("Edit", "smooth", "Smooth",
     '<path d="M3 16c3 0 3-9 6.5-9s3.5 9 6.5 9 3-6 5-6"/>'),
    ("Edit", "group", "Group",
     '<rect x="3" y="3" width="18" height="18" rx="2" stroke-dasharray="3 3"/><rect x="7" y="7" width="5" height="5" rx="0.5"/><rect x="13" y="12" width="4" height="5" rx="0.5"/>'),
    ...
    ("Material & light", "palette", "Color",
     '<circle cx="13.5" cy="7" r="1"/><circle cx="17.5" cy="11" r="1"/><circle cx="7" cy="8" r="1"/><circle cx="6.5" cy="13" r="1"/><path d="M12 3a9 9 0 1 0 0 18 1.8 1.8 0 0 0 1.7-2.4c-.2-.5-.2-1.1 0-1.6.3-.8 1-1.3 1.9-1.3h1.4A4 4 0 0 0 21 11.9 9 9 0 0 0 12 3Z"/>'),
    ("Material & light", "texture", "Texture",
     '<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9c3-2 6 2 9 0s6-2 9 0"/><path d="M3 15c3-2 6 2 9 0s6-2 9 0"/>'),
    ("Material & light", "opacity", "Opacity",
     '<circle cx="12" cy="12" r="9"/><path d="M12 3a9 9 0 0 1 0 18Z"/>'),
    ...
    ("3D printing", "nozzle", "Nozzle",
     '<path d="M8 4h8l-1.2 7H9.2z"/><path d="M9.2 11 12 16l2.8-5"/><path d="M11.3 16h1.4"/>'),
    ("3D printing", "layer-height", "Layer height",
     '<path d="M4 7h10"/><path d="M4 12h10"/><path d="M4 17h10"/><path d="M20 8v8"/><path d="M18 10l2-2 2 2"/><path d="M18 14l2 2 2-2"/>'),
    ("3D printing", "overhang", "Overhang warning",
     '<path d="M12 3 22 19H2Z"/><path d="M12 10v4"/><path d="M12 17h.01"/>'),
    ("3D printing", "fit-plate", "Fit to plate",
     '<path d="M3 18h18"/><rect x="9" y="9" width="6" height="6" rx="0.5"/><path d="M6 6 3 6 3 9"/><path d="M18 6 21 6 21 9"/>'),
    ("3D printing", "print-time", "Print time",
     '<circle cx="12" cy="13" r="8"/><path d="M12 9v4l2.5 2"/><path d="M9 2h6"/>'),

    # ---- Navigation ----
    ("Navigation", "home", "Home",
     '<path d="M3 11 12 3l9 8"/><path d="M5 9.5V20h14V9.5"/><path d="M10 20v-6h4v6"/>'),
    ("Navigation", "search", "Search",
     '<circle cx="11" cy="11" r="7"/><path d="M21 21l-4.3-4.3"/>'),
    ("Navigation", "folder", "Projects",
    ...
    ("General", "eye", "Show / hide",
     '<path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/>'),
    ("General", "lock", "Lock",
     '<rect x="5" y="11" width="14" height="9" rx="2"/><path d="M8 11V8a4 4 0 0 1 8 0v3"/>'),
    ("General", "trash", "Delete",
...
def main():
    ...
    # Figma importer data: inner paths only (the importer wraps + bakes color).
    data = [[cat, name, label, inner] for cat, name, label, inner in ICONS]
    with open(os.path.join(OUT, "_icons.js"), "w") as f:
        f.write("const ICONS = " + json.dumps(data) + ";\n")
...
<header>
  ...
  language, so these drop in beside Lucide. The 3D-printing group = custom glyphs no standard set ships.</p>
</header>
{''.join(sections)}
...
"""
    ...
    print(f"Wrote {total} SVGs + preview.html + _icons.js to {OUT}")
