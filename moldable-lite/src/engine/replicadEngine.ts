...
export class ReplicadEngine implements Engine {
  ...
  private importKind: "step" | "stl" = "step";
  ...
  async setImport(file: Blob | null, kind: "step" | "stl" = "step"): Promise<void> {
    ...
    const r = await this.withWatchdog(this.api.importShape(file, kind), 45_000);
  ...
  private respawn() {
    ...
    if (this.importBlob) void this.api.importShape(this.importBlob, this.importKind);
  }
  ...
  async build(input: BuildInput): Promise<EngineResult> {
    ...
    try {
      res = await this.withWatchdog<WorkerBuildResult>(this.api.build(input.code, input.params, input.ops, input.preview ? { probeLimit: false, coarse: true } : input.probe ? { probeLimit: false, coarse: false } : undefined) as unknown as Promise<WorkerBuildResult>);
    } catch (timeoutErr) {
      this.respawn();
      throw timeoutErr;
    }
